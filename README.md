...
1. **Server in Codex registrieren**
   ...
   codex mcp add coding-advisors \
     ...
     npx -y -p @meinzeug/mcp-ai-bug-helper mcp-ai-bug-helper
...
- **Amp CLI** – `amp mcp add coding-advisors -- npx -y -p @meinzeug/mcp-ai-bug-helper mcp-ai-bug-helper`
- **Claude Code CLI** – `claude mcp add coding-advisors npx -y -p @meinzeug/mcp-ai-bug-helper mcp-ai-bug-helper`
- **Codex CLI** – `codex mcp add coding-advisors --env "OPENROUTER_API_KEY=sk-or-v1-..." npx -y -p @meinzeug/mcp-ai-bug-helper mcp-ai-bug-helper`
- **Copilot CLI** – `/mcp add` → Name `coding-advisors`, Command `npx -y -p @meinzeug/mcp-ai-bug-helper mcp-ai-bug-helper`
- **VS Code / Copilot** – `code --add-mcp '{"name":"coding-advisors","command":"npx","args":["-y","-p","@meinzeug/mcp-ai-bug-helper","mcp-ai-bug-helper"]}'`
...
- **Windows 11 & Codex** – falls `npx` Chrome oder Node aus `Program Files` laden muss, ergänze in `~/.codex/config.toml`:
  ...
  command = "cmd"
  args = ["/c", "npx", "-y", "-p", "@meinzeug/mcp-ai-bug-helper", "mcp-ai-bug-helper"]
